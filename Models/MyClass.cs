...
public class BaseInfo
{
    ...
    [ExcludeFromExcelImport] 
    [Display(Name = "Updated By", Order = 91)] public string UpdatedBy { get; set; }
...
public class Bus : BaseInfo, IBaseMethod
{
    ...
    public List<string> ConnectedBus { get; set; } // Tags of the connected bus if any through tie-breaker, busbar etc. 
    ...
    public string GeneratedFrom { get; set; }  // newly generated bus based on the connections logics 
...
public class Branch : BaseInfo
{
    ...
    [Required] [Display(Name = "From Element Tag")]
    [RegularExpression(@"^([ .\/\a-zA-Z0-9]){3,100}$",
        ErrorMessage = "From Element Tag should be 3 to 100 characters and can not contain any special characters")]
    public string FromElement { get; set; } // Can be Bus, Branch, or Non-Branch Element tag

    public string? FromBus { get; set; } = null; // Bus From Tag

    [Required] [Display(Name = "To Element Tag")]
    [RegularExpression(@"^([ .\/\a-zA-Z0-9]){3,100}$",
        ErrorMessage = "To Element Tag should be 3 to 100 characters and can not contain any special characters")]

    public string ToElement { get; set; } // Can be Bus, Branch, or Non-Branch Element tag

    public string? ToBus { get; set; } = null; // Bus To Tag
    ...
    [ExcludeFromExcelExport]
    public Complex Io { get; set; } // Branch Operating Current in PU FromBus to ToBus
    [ExcludeFromExcelExport]
    public float KW { get; set; } // Branch Operating KW
    [ExcludeFromExcelExport]
    public float KVAR { get; set; } // Branch Operating KVAR
    [ExcludeFromExcelExport]
    public float VRatio { get; set; } = 1; // Ratio of FromBus rated voltage to ToBus rated voltage (applicable for transformer) 
    //public float SCkAaMax { get; set; }   // Max SC in kA Actual for SLD Display, Contributing to FromBus. Contribution to ToBus = -1 * VRatio 
    //public float SCkAaMin { get; set; }   // Max SC in kA Actual for SLD Display, Contributing to FromBus. Contribution to ToBus = -1 * VRatio 
    public Branch()
    {
    }
...
public class Transformer : Branch
{
    ...
    public float V1 { get; set; } // From (1) side (FromBus) Open Circuit Voltage 
    [Display(Name = "Secondary Voltage", Order = 18)]
    public float V2 { get; set; } // To (2) (ToBus) side Open Circuit Voltage

    // FromBus and ToBus already defined
    //public string FromBus { get; set; }     // Bus V1 (From side) Tag, it would be node
    //public string ToBus { get; set; }     // Bus V2 (To side) Tag, it would be node
...
public class BusBarLink: BaseInfo
{
    [Required] [Display(Name = "From Element Tag")]
    [RegularExpression(@"^([ .\/\a-zA-Z0-9]){3,100}$",
        ErrorMessage = "From Element Tag should be 3 to 100 characters and can not contain any special characters")]
    ...
    public string ToElement { get; set; }
    public float RatedCurrent { get; set; }
    // This is a zero-resistance link between buses or bus-branch
}
...
public class Switch : BaseInfo
{
    ...
    [RegularExpression(@"^([ .\/\a-zA-Z0-9]){3,100}$",
        ErrorMessage = "From Element Tag should be 3 to 100 characters and can not contain any special characters")]
    public string FromElement { get; set; } // Connected Element Tag no, bus or non-bus element
    ...
    public string SwitchType { get; set; }  // Isolator, CircuitBreaker, etc.
}
...
public class Load : BaseInfo
{
    ...
    public Load(string loadType, string tag, string bft, Complex scpu, float vr, float dr)
    {
        UID = Guid.NewGuid();
        LoadType = loadType;
        Tag = tag;
    ...
    public string LoadType { get; set; } = "Load"; // Motor, Heater, Capacitor, LumpLoad, etc.
    ...
    [Display(Name = "Connected Bus Tag", Order = 22)]
    public string ConnectedBus { get; set; } // Source (From) Bus - Assigned Power Source Unique Switchboard Bus Tag e.g., SS08-PMCC-04-BusA
